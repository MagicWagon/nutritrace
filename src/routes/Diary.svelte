                ...
                <button class="diary-item-btn" on:click={() => selectMode ? toggleItemSelect(item) : openEditItem(item)}>
                  ...
                  <div class="item-info">
                    ...
                    <span class="item-meta text-3 text-sm">
                      {item.portion || item.amount || 100}{item.unit || 'g'}{#if Math.abs((item.quantity || 1) - 1) > 0.001} × {item.quantity || 1}{#if $diaryShowPortionSize} (= {Math.round((item.portion || item.amount || 100) * (item.quantity || 1) * 10) / 10}{item.unit || 'g'}){/if}{/if}
